...
# 3.定时执行：向sina拿数据，比较since_id参数，若有更新过的微博，抓下来，按格式插入updates，最后更新since-id
# 同理，向客户端返回时候，返回since-id比传上来的大的条目；暂定为只返回每种平台最新的10条（有的话）
...
social-network-update = # yoyo server端数据
  ...
  # updates:
  #   * update-time: 'UTC'
  #   	is-requested: false	# 客户端是否已经请求过，保留，暂时没实现
